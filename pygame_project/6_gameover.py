...
# 공 크기에 따른 최초 스피트
ball_speed_y = [-22, -19, -16, -13] # index 0, 1, 2, 3, 에 해당하는값
...
# 게임 종료 메시지 / Timeout, Mission complete(성공), Game Over (캐릭터 공에 맞음, 실패)
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3: # 가장 작은 크기의 공이 아니라면 다음 단계의 공으로 나누어주기

                    ball_width = ball_rect.size[0]
                    ball_height = ball_rect.size[1]
                    
                    # 나눠진 공의 정보
                    small_ball_rect = ball_images[ball_img_idx +1].get_rect() # 현재 공의 idx보다 하나 더 많은 공이 더 작은 공임
                    small_ball_width = small_ball_rect.size[0]
                    small_ball_height = small_ball_rect.size[1]

                    balls.append({ # 왼쪽으로 튕기는 공
                        "pos_x" : ball_pos_x + (ball_width/2) - (small_ball_width/2), # 현재 공위치로 부터 시작되어야함.
                        "pos_y" : ball_pos_y + (ball_height/2) - (small_ball_height/2), # 현재 공위치로 부터 시작되어야함.
        ...
        #         if 충돌하면:
        #             break
        #     else: // for문 안쪽에서 break 걸리지 않으면 여기로 와서 continue로 넘어감.
        #         continue
        #     break // 만약 break 걸렸다면 else를 타지 않고 break를 타서 바깥 for 문까지 탈출!
    ...
    timer = game_font.render("Time : {}".format(int(total_time - elpased_time)),True, (255,255,255)) 
